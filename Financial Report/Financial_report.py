llm_config = {
    "model": "gpt-4o", 
    "api_key": "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"
    }
...
writing_tasks = [
    """Compose a detailed financial report using all available data, incorporating the normalized_prices.png figure 
    and any additional provided visuals.  
    Present a structured comparison of fundamental ratios and key financial metrics in a table with insightful commentary.  
    Conduct a comparative analysis of the stocks, assessing their correlation, associated risks, and overall performance.  
    Summarize and evaluate recent news developments for each stock, highlighting key headlines and their potential impact.  
    Establish connections between fundamental ratios and news events, explaining their influence on stock performance.  
    Provide a forward-looking analysis of potential future scenarios based on financial data, market trends, and news insights."""
]
...
writer = autogen.AssistantAgent(
    ...
    llm_config=llm_config,
    system_message="""
        You are a professional writer renowned for crafting insightful and engaging financial reports.
        Your narratives simplify complex financial concepts into compelling, clear analyses.
        Integrate all provided metrics into your analysis and ensure that your report is comprehensive.
        Respond solely with the final financial report in markdown format without any markdown language block indicators.
        Do not include any extraneous commentary beyond the report itself.
    """
)
...
legal_reviewer = autogen.AssistantAgent(
    name="Legal_Reviewer",  # Replace space with an underscore
    llm_config=llm_config,
...
consistency_reviewer = autogen.AssistantAgent(
    name="Consistency_Reviewer",  # Replace space with an underscore
    llm_config=llm_config,
...
textalignment_reviewer = autogen.AssistantAgent(
    name="Text_Alignment_Reviewer",  # Replace space with underscores
    llm_config=llm_config,
    system_message="You are a text data alignment reviewer, known for "
        ...
        "You must ensure that the text clearly describes the numbers provided in the text "
...
completion_reviewer = autogen.AssistantAgent(
    name="Completion_Reviewer",  # Replace space with an underscore
    llm_config=llm_config,
...
meta_reviewer = autogen.AssistantAgent(
    name="Meta_Reviewer",  # Replace space with an underscore
    llm_config=llm_config,
...
assets = st.text_input("Stocks you want to analyze (provide the tickers sep by comma)?")
